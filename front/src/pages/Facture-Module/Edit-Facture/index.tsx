...
function main() {
    ...
    useEffect(() => {
        factureService.getById(id??'').then(x=>reset(x.data)).catch(()=>alert("an error occured"));
    ...
    const formSubmit = async (event: React.ChangeEvent<HTMLFormElement>) => {
        ...
        if (!result) {
            ...
            factureService.updateFacture(id??'',data).then((result: any) => {
                console.log(result);
                if (result) {
    ...
    return (
        <>
            <div className="flex items-center mt-8 intro-y">
                <h2 className="mr-auto text-lg font-medium">Edit Invoice</h2>
            </div>
